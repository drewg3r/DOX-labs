...
class Lab4:
    ...
    def student_crit_check(self):
        ...
        yd = [0, 0, 0, 0]  # змінна yd містить в собі значення функції відгук
        # для перших чотирьох рядків матриці планування
        # експерименту. Ці значення використовуються в
        # подальшій перевірці критерія Фішера а також
        # виводяться у консоль для перевірки
        for i in range(4):
            if t[i] < t_tabl:
                nm.append(i)
                d += 1
